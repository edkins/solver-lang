...
def parse_statement(ast: Ast, env:Env) -> tuple[Statement]:
    if isinstance(ast, lark.Tree):
        if ast.data == 'def':
            ...
            xs = [parse_nametype(x) for x in nametypes.children]
            for x in xs:
                if x.name in env:
                    return Erroneous((_def.start_pos, _close.end_pos))
                env[x.name] = x
            es:list[Expr] = []
